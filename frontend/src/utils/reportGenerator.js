...
const DEFAULT_SIGNATURE_VALUE = '________________';

export const collectSignatureInfo = () => {
  if (typeof window === 'undefined') {
    return {
      generatedBy: DEFAULT_SIGNATURE_VALUE,
      checkedBy: DEFAULT_SIGNATURE_VALUE,
      approvedBy: DEFAULT_SIGNATURE_VALUE
    };
  }
  ...
  const prompts = [
    {
      key: 'generatedBy',
      message: 'Enter the name of the person generating/downloading this report:'
    },
    {
      key: 'checkedBy',
      message: 'Enter the name of the person who checked this report:'
    },
    {
      key: 'approvedBy',
      message: 'Enter the name of the person who approved this report:'
    }
  ];
  ...
  for (const promptConfig of prompts) {
    const value = window.prompt(promptConfig.message);
    if (value === null) {
      return null;
    }
    signatureInfo[promptConfig.key] = value.trim() || DEFAULT_SIGNATURE_VALUE;
  }

  return signatureInfo;
};

export const buildSignatureFooter = (signatureInfo) => {
  if (!signatureInfo) return '';
  return [
    `Generated by: ${signatureInfo.generatedBy} (report downloader)`,
    `Checked by: ${signatureInfo.checkedBy}`,
    `Approved by: ${signatureInfo.approvedBy}`
  ].join('\n');
};

const API_BASE = process.env.NEXT_PUBLIC_API_URL || '';

export const generateReport = async (reportId, data, providedSignatureInfo = null) => {
  const signatureInfo = providedSignatureInfo ?? collectSignatureInfo();
  if (!signatureInfo) {
    return;
  }
